...
def build_model():
    ...
    #The parameters in the pipeline is obtained by GridSearch trained in the Jupyter Notebook
    pipeline = Pipeline([('vect',CountVectorizer(tokenizer=tokenize)),
                         ('tfidf',TfidfTransformer()),
                         ('MOC', MultiOutputClassifier(RandomForestClassifier(criterion= 'gini',n_estimators= 100,max_features=5000)))])
#     parameters = {
#     'MOC__estimator__n_estimators': [100,200],
#     'MOC__estimator__criterion':['gini','entropy'] #MOC__criterion
#     }

#     cv =  GridSearchCV(pipeline, parameters,  verbose=3,  n_jobs=-1) #scoring='f1_micro',
#     return cv
    return pipeline
